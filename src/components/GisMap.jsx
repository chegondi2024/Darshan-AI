...
import { ShieldCheck, MapPin, Users, Zap, AlertTriangle, Landmark, Footprints, Train, Bus, Car, DoorOpen, Wallet, Ticket, Eye, Clock } from 'lucide-react';
...
const createNeuralNodeIcon = (name, color, health) => {
  const html = renderToString(
    <div className="flex flex-col items-center">
      ...
      <div className="mt-3 bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-2xl border border-slate-100 flex flex-col items-center min-w-[80px] shadow-[0_10px_30px_rgba(0,0,0,0.1)]">
         <span className="text-[9px] font-black text-slate-900 uppercase tracking-tighter leading-none mb-1">{name}</span>
...
const createStatusIcon = (status, iconColor, type, badge) => {
  const pulseSpeed = status === 'CRITICAL' ? '1s' : status === 'HEAVY' ? '2s' : '4s';
  const html = renderToString(
    <div className={`relative flex items-center justify-center`}>
      {status !== 'NORMAL' && (
         <div className="absolute w-16 h-16 rounded-full border-2 border-white opacity-20 animate-ping" style={{ backgroundColor: iconColor, animationDuration: pulseSpeed }}></div>
      )}
      <div className="absolute w-12 h-12 rounded-full border border-white opacity-10 animate-pulse" style={{ backgroundColor: iconColor }}></div>
      ...
      <div className={`w-9 h-9 rounded-full border-2 border-white shadow-2xl flex items-center justify-center transition-all bg-white overflow-hidden relative`}>
         {type === 'station' ? <Train size={18} /> : 
          ...
          type === 'vehicle' ? <Car size={16} style={{ color: iconColor }} /> :
          <Users size={18} style={{ color: iconColor }} />}
      </div>
      
      {badge && (
         <div className="absolute -bottom-2 whitespace-nowrap z-50">
            <div className={`text-[7px] font-black text-white px-1.5 py-0.5 rounded-full border border-white shadow-xl ${
               status === 'CRITICAL' || status === 'FULL' ? 'bg-red-500' : 'bg-slate-800'
            }`}>
               {badge}
            </div>
         </div>
...
const VehicleFlow = ({ path, color, density }) => {
   ...
   return <Marker position={path[posIndex]} icon={createStatusIcon('NORMAL', color, 'vehicle')} zIndexOffset={1000} />;
};
...
const GisMap = ({ mapView, activeNodeId, crowdData, trafficData, lockerData, advisoryData, darshanData, showGhatTraffic, showPACMarkers, showCrowdMarkers, showGoogleTraffic, activeRoute, transitFleet, sector = 'tirupati' }) => {
    ...
    <div className="w-full h-full relative">
      <MapContainer center={mapView.center} zoom={mapView.zoom} className="w-full h-full" zoomControl={false}>
        ...
        {visibleNodes.map(node => {
          ...
          // Special Color Mapping for Global Hubs to match Sidebar
          if (isGlobalHub) {
             const colors = {
                tirupati: '#eab308',
                vijayawada: '#10b981',
             ...
             statusColor = colors[node.id] || statusColor;
          }

          const healthVal = status?.info?.split('%')[0] || '100';

          return (
            <React.Fragment key={node.id}>
               <DensityHeatmap coordinates={node.coords} status={status?.status} />
               <Marker 
                  position={node.coords} 
                  icon={isGlobalHub ? createNeuralNodeIcon(node.name.split(' ')[0], statusColor, healthVal) : createStatusIcon(status?.status || 'NORMAL', statusColor, node.type, status?.count || status?.info)}
               >
                 ...
                 <Popup className="tactical-popup">
                    <div className="p-4 min-w-[200px] bg-white rounded-2xl shadow-2xl border border-slate-100">
                       <div className="flex items-center gap-2 mb-3 border-b border-slate-100 pb-2">
                          <div className={`w-2 h-2 rounded-full animate-pulse`} style={{ backgroundColor: statusColor }}></div>
                          <div className="text-[11px] font-black text-slate-900 uppercase tracking-widest">{node.name}</div>
                       </div>

                       {/* TICKET-WISE WAITING INTELLIGENCE */}
                       {((isGlobalHub ? darshanData?.[node.id]?.categories : darshanData?.categories)) && (
                          <div className="flex flex-col gap-2 mb-4 bg-slate-50 p-2 rounded-xl border border-slate-100">
                             ...
                             {(isGlobalHub ? darshanData?.[node.id]?.categories : darshanData?.categories).map((cat, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-white px-3 py-2 rounded-lg shadow-sm border border-slate-100">
                                   <div className="flex items-center gap-2">
                                      {cat.icon === 'Users' ? <Users size={12} className="text-yellow-600" /> : 
